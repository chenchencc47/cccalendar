...
[void]$sb.AppendLine("public class MouseSimD {")
$q = [char]34
...
# Per screenshot visual measurement: calendar grid starts ~x=410, grid right edge ~x=1174, 7 columns => width 764, each cell 109.
# Row 5 (24..30): y 598..710. Center y = 654.
# Column 7 (Sunday 30): x = 410 + 6*109 + 54 = 1118.
$tx = 1118
$ty = 654
Write-Output (("Screenshot-measured Aug30 center=" + $tx + "," + $ty))
[MouseSimD]::ClickAt($tx, $ty)
Start-Sleep -Milliseconds 600
$p = [System.Diagnostics.Process]::GetProcessById($PidOfApp)
$root = [System.Windows.Automation.AutomationElement]::FromHandle($p.MainWindowHandle)
$all = $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)
$sel=$false; $anySel=$null
foreach ($e in $all) {
  $n=$e.Current.Name
  if ($n -like "*CalendarDayViewModel*" -and $n -like "*IsSelected = True*") { $anySel = $n; if ($n -like "*Date = 2026/8/30*") { $sel=$true } }
}
Write-Output (("Any selected snippet=" + $(if($null -ne $anySel){$anySel.Substring(0,[Math]::Min(200,$anySel.Length))} else {"NONE"})))
Write-Output (("Aug30 selected=" + $sel))
$ts1 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp1 = "D:/myProgram/cccalendar/screenshots/07_qa_click_vcoord_aug30_" + $ts1 + ".png"
Take-Screenshot -Path $sp1
Write-Output (("SCT_VCOORD_CLICK=" + $sp1))

# Double click same point
[MouseSimD]::DoubleClickAt($tx, $ty)
Start-Sleep -Milliseconds 1100
$ts2 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp2 = "D:/myProgram/cccalendar/screenshots/08_qa_dbclick_vcoord_aug30_" + $ts2 + ".png"
Take-Screenshot -Path $sp2
Write-Output (("SCT_VCOORD_DBCLICK=" + $sp2))
...
if ($null -eq $Combo) {
  Write-Output "NO_QUICK_WINDOW: vcoord no combo"
} else {
  ...
  if ($null -ne $anc) {
    ...
    function DumpD { param($e,$d=0,$md=7) if ($null -eq $e) {return} $pad=""; for($i=0;$i -lt $d;$i++){$pad+="  "} $n=$e.Current.Name; if([string]::IsNullOrEmpty($n)){$n="(empty)"} $t=$e.Current.ControlType.ProgrammaticName -replace "^ControlType..",""; Write-Output ($pad + $t + " " + [char]34 + $n + [char]34); if ($d -ge $md){return} $ch = $e.FindAll([System.Windows.Automation.TreeScope]::Children, [System.Windows.Automation.Condition]::TrueCondition); foreach($c in $ch){ DumpD $c ($d+1) $md } }
    DumpD $anc 0 7
  ...
  if ($null -ne $CancelBtn) {
    ...
    [MouseSimD]::ClickAt([int]($cbR.Left + $cbR.Width/2), [int]($cbR.Top + $cbR.Height/2))
  ...
  $sp3 = "D:/myProgram/cccalendar/screenshots/09_qa_cancel_vcoord_aug30_" + $ts3 + ".png"
  Take-Screenshot -Path $sp3
  Write-Output (("SCT_CANCEL_VCOORD=" + $sp3))
}