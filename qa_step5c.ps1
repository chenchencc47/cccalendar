...
[void]$sb.AppendLine("public class MouseSim2 {")
$q = [char]34
...
# Target date 2026/9/5 (future, next month) - rect 1045,775,89,90 -> center 1090,820
$tx = 1045 + 44
$ty = 775 + 44
Write-Output (("Target center=" + $tx + "," + $ty))
# 5a: single click to select
[MouseSim2]::Click($tx, $ty)
Start-Sleep -Milliseconds 500
$ts1 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp1 = "D:/myProgram/cccalendar/screenshots/07_qa_click_sep5_" + $ts1 + ".png"
Take-Screenshot -Path $sp1
Write-Output (("SCREENSHOT_CLICK=" + $sp1))
# Verify IsSelected = True for target DataItem
$all = $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)
$sel=$false
foreach ($e in $all) {
  $n=$e.Current.Name
  if ($n -like "*CalendarDayViewModel*" -and $n -like "*Date = 2026/9/5*" -and $n -like "*IsSelected = True*") { $sel=$true; break }
}
Write-Output (("IsSelected after click=" + $sel))
# 5b: real DoubleClick at same point
[MouseSim2]::DoubleClick($tx, $ty)
Start-Sleep -Milliseconds 900
$ts2 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp2 = "D:/myProgram/cccalendar/screenshots/08_qa_doubleclick_quickadd_sep5_" + $ts2 + ".png"
Take-Screenshot -Path $sp2
Write-Output (("SCREENSHOT_DOUBLECLICK=" + $sp2))

# Check quick window presence (ComboBox anywhere)
...
if ($null -eq $Combo) {
  Write-Output "NO_QUICK_WINDOW: Double click did NOT trigger quick-add window"
} else {
  Write-Output "QUICK_WINDOW_OPENED: yes"
  # Dump combo selected text (Text children)
  ...
  foreach ($t in $allTxt) { Write-Output (("  Combo text child=" + $t.Current.Name)) }
  $anc = $Combo
  ...
  if ($null -ne $anc) {
    ...
    function Dump2 { param($e,$d=0,$md=7) if ($null -eq $e) {return} $pad = ""; for($i=0;$i -lt $d;$i++){$pad += "  "} $n=$e.Current.Name; if([string]::IsNullOrEmpty($n)){$n="(empty)"} $t=$e.Current.ControlType.ProgrammaticName -replace "^ControlType..",""; Write-Output ($pad + $t + " " + [char]34 + $n + [char]34); if ($d -ge $md){return} $ch = $e.FindAll([System.Windows.Automation.TreeScope]::Children, [System.Windows.Automation.Condition]::TrueCondition); foreach($c in $ch){ Dump2 $c ($d+1) $md } }
    Dump2 $anc 0 7
  ...
  if ($null -ne $CancelBtn) {
    ...
    [MouseSim2]::Click([int]($cbR.Left + $cbR.Width/2), [int]($cbR.Top + $cbR.Height/2))
  ...
  $sp3 = "D:/myProgram/cccalendar/screenshots/09_qa_cancel_after_doubleclick_" + $ts3 + ".png"
