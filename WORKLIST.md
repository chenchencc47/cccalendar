...
**唯一下一步**：无待开发项。可选方向见下方「尚未完成的事项」。

**说明**：本文件下方的「当前检查点」逐条日志跨越 2026-08 至 2026-09 多个时期，**按时间倒序但混放了不同阶段的快照**，仅作历史记录；需要判断"现在做到哪了"请只看本段与各 P 章节的 `[x]/[ ]` 标记。

## 尚未完成的事项

- **桌面组件小字号（已完成，见 P64）**：原"延期"结论在调查后被推翻——它不是"字号偏小"，而是**固定 px 不跟随设置**。已改为按比例派生，问题已解决，此处仅留结论备查。
...
- ~~View 级区块间距重排~~ **已完成（见 P65）**：页面级留白已统一为 `24,20,24,24` 并由契约测试守住。行内间距（图标→文字等 4/8px）刻意不并入基线，理由见 P65。
- **长期并行**：P15-09d 两地云端验收；生产 HTTPS/OIDC（P47）与角色矩阵、令牌撤销（见 `docs/VERSION_EVOLUTION_PLAN.md`）。
- **文档陈旧**：已清掉一批（见 P63），仍有零星历史文档版本号可能滞后。

## 安全事项（重要）

- 团队共享口令与 DevToken 签名密钥曾以**明文**出现在 `docs/CLOUD_OPS_GUIDE.md`、`WORKLIST.md` 与 `docs/USER_GUIDE - 副本.md`，且本仓库为**公开仓库**。2026-09-16 已从当前工作树全部移除（密钥改为从服务器 systemd 环境变量读取），含口令的过期副本文件已删除。
- **但 git 历史中仍可检出这些值**（提交 `a9ec1f9`、`73a0e40`）。从历史里彻底抹除需要 `git filter-repo` 重写并强推，会打断所有克隆。
- **唯一彻底的补救是按 `docs/CLOUD_OPS_GUIDE.md` §场景 F 轮换 `Authentication__DevToken__SharedSecret` 与 `SigningKey`**，然后通知全体成员重新登录。
- **决策记录（2026-09-16）**：用户判断"只是写进项目里"，决定**暂缓轮换**。本项据此记为**已知并接受的风险**，不再重复提醒。需要时按上面步骤执行即可；建议在团队正式长期使用（或服务器转为生产 HTTPS/OIDC）前完成轮换。
...
- [x] P61-06b 留白与层级重排（控件高度档位）。
  - `UiControlHeight` 32 → **36**，向 UI_DESIGN §2.2「主要输入 36px」档对齐（该档此前虽已声明 `ControlHeightPrimary=36` 却无人消费）。紧凑档 `UiCompactControlHeight` 保持 28，用于工具栏与分段控件。参考实现的留白明显更大（菜单项最小高 40、弹窗内边距 24），本项目历史上偏紧凑，这一步先把正文控件抬到 36。
  - **踩到一个真实回归（值得记录）**：全局抬高的副作用是 `RoomBookingPicker` 的日期导航行也从 32 长到 36，看板顶边被整体下移 3.24px，触发 `RoomBookingPickerRuntimeTests.BoardIsLaidOutBelowHeaderAndHitTestsToItself` 失败（该测试把「板顶边 = 32 + 8 + 26 = 66」当作契约）。
    - 第一次修法（只给导航按钮与导出按钮钉 28）反而变成 61.24——因为该行是 `Auto`，行高由最高的子元素决定，逐个子元素钉高并不能稳定行高。
    - 最终修法：**把三个行高显式写死**（第 0 行 32、间隔 8、表头 26），让看板几何不再受全局留白调整牵连。这也让该处布局从"隐式依赖子元素"变成"显式声明契约"。
...
- [x] P65 统一页面级留白。
  - 先做了**量化盘点**而不是凭感觉调：统计全部 `Margin` 值后发现，出现最多的 `8,0,0,0`(53 次)、`4,0,0,0`(26 次)、`6,0,0,0`、`0,4,0,0` 全是**行内间距**（图标→文字、复选框→标题、色条→文本）。UI_DESIGN §2.2 的 4/8/12/16/24 基线管的是**区块级**间距，强行把这些行内值也归并会把密集行（按钮组、列表项、检查框行）撑肿。因此本切片**只处理区块级留白**。
  - 实测区块级留白（四边均 ≥12 的页面外边距）有 **4 种值并存**：`24,20,24,24`（7 页）、`28,24,28,28`（助手页）、`24,20,32,32`（设置页 5 处）、`22,18,22,16`（编辑日程 / 当日日程窗口）。切页时内容起始位置会横向跳动，这是真正可见的不一致。
  - 统一为 `24,20,24,24`（取多数值），共改 **9 处**。归一化后 15 处页面级留白全部一致。
  - **附带修掉一个真实观感问题**：助手页原先横向 28px、右侧还有额外压力，在 1920 宽下 `重试上次请求` 按钮与窗口右边缘贴得过近显得像被裁切；改为 24px 后该按钮已与边缘留出余量（截图核验）。
  - Red→Green：新增契约测试 `PageLevelPaddingIsUniformAcrossViews`（扫描 11 个页面/窗口，**只校验区块级留白**，并在注释中写明行内间距为何不在管辖范围——避免后来者把 4px/8px 的图标间距也"修正"掉）。
  - Verify：`eng\verify.cmd` 退出 0；format check 通过；build **0 警告 0 错误**；完整回归 **521/521**（Core 93、Infrastructure 104、Desktop 280、Server 44）；浅色 1366×768 九页截图核验，无横向跳动或截断。
...
- [x] P63-01 校正陈旧版本号与已过时的状态描述（影响判断的优先修）。
  - `docs/AUTO_UPDATE_DESIGN.md`：原文写「当前版本 `0.4.5`…尚未接入静默安装、代码签名校验和回滚」。实际 `0.6.1` 起已实现**下载后校验 SHA-256 → 自动关闭 → 安装 → 重启**。已改为「当前版本 0.6.6，一键安装已完成；仍未完成的是代码签名（Authenticode）校验、灰度发布与失败回滚」——保住原文档"还没做什么"的意图，但不再说错已实现的部分。
  - `docs/UPDATE_AND_COLOR_GUIDE.md`：当前发布版本 `0.4.5` → `0.6.6`。
  - `docs/TEAM_MEMBER_GUIDE.md`：安装包示例 `cccalendar-0.4.5-...exe` → `0.6.6`。
  - `docs/CLOUD_OPS_GUIDE.md`：头部与客户端章节的"Windows 客户端 0.5.2"改为"以 OSS 清单 `version.json` 为准"，避免每次发版都要改文档。
  - `docs/DESIGN.md`：头部「P0-P14 已完成…按 P15 逐步实现，最后更新 2026-08-17」→ 当前版本与阶段，并指向 UI 优化方案与版本路线。
  - `docs/SYNC_DESIGN.md`：头部「按 P15 逐步实现」→ 说明 P15 会议室协作切片已上线、个人数据同步仍在后续阶段。
- [x] P63-02 `WORKLIST.md` 新增「当前权威状态」段落。
  - 原「当前检查点」逐条日志跨越 2026-08 至 2026-09 多个时期、混放了不同阶段的快照（同时出现"当前阶段 P18""当前发布版本 0.4.14""NEXT 跑 0.3.3"等互相矛盾的陈述），无法据此判断进度。
  - 未逐条重写 1200 行账本（那是历史记录，有价值），而是在文件顶部加一段**唯一可信的状态表**（版本/基线/阶段/阻塞项/安全待办 + 唯一下一步），并明确说明下方日志仅作历史、判断进度只看本段与各 P 章节的 `[x]/[ ]`。
- [x] P63-03 **安全：删除含明文口令的过期文档副本**。
  - 修正一处我先前的错误结论：`docs/USER_GUIDE - 副本.md` 我上次说"未被跟踪"，实际**它被跟踪**，其中的团队共享口令一直在公开仓库的当前树里（早前那次扫描因为文件名含空格与转义，被我的过滤脚本漏掉了）。
  - 处理：`git rm` 删除该文件（它是 0.5.2 期副本、已被 `USER_GUIDE.md` 取代，且 `UI_OPTIMIZATION_PLAN.md` §9 早已标记为"superseded"）。删除后 `git grep` 确认**当前跟踪树中已无共享口令、也无签名密钥**。
  - **仍然必须做的事**：这些值存在于 git 历史（提交 `a9ec1f9`、`73a0e40`）。彻底抹除需 `git filter-repo` 重写历史并强推，会打断所有克隆；唯一干净的补救是**按 `CLOUD_OPS_GUIDE.md` §场景 F 轮换 `SharedSecret` 与 `SigningKey`**。已写入 WORKLIST 顶部「安全事项」。
- Verify：`eng\verify.cmd` 退出 0；build 0 警告 0 错误；完整回归 **515/515**。
...
- [x] P60-01 令牌基线：在 `Theme.xaml` 新增全部令牌（间距 `SpacingXs/Sm/Md/Lg/Xl`、圆角 `RadiusSm/Md/Lg`、字号 `FontCaptionSize/FontCompactSize/FontPanelTitleSize/FontPageTitleSize/FontClockSize`、控件尺寸 `ControlHeightPrimary/IconButtonSize/SidebarWidth/BrandBarHeight/NavigationItemHeight`、动效 `MotionFast/MotionBase/EasingStandard`、新增语义画刷 `ScrollbarThumbBrush/ScrollbarThumbHoverBrush/FocusRingBrush/OverlayScrimBrush/ElevationPanelBrush/ElevationProminentBrush`），`DarkTheme.xaml` 同步覆写颜色键。**本切片不改任何页面引用**，观感与改动前一致。
  - Red：新增 `tests\CcCalendar.Desktop.Tests\Views\ThemeTokenContractTests.cs`（6 项）。与既有 `UiDesignContractTests` 的纯文本断言不同，本测试在 `WpfRuntimeHost` 的 STA 线程上**真实加载** `Theme.xaml` 与 `DarkTheme.xaml` 资源字典再断言，因为「键写在文件里」不等于「运行时能读出」——键重复、XML 非法或类型错误都会让文本断言误判通过。首次运行 **4 失败 / 2 通过**：`ThemeDeclaresEverySpacingRadiusFontAndControlSizeToken`（间距令牌 `SpacingXs` 未声明）、`ThemeDeclaresMotionTokens`、`DarkThemeOverridesEverySemanticColorToken`、`RadiusTokensStayWithinTheDocumentedEightPixelCeiling` 均按预期失败；`ThemeDeclaresEveryDynamicResourceKeyItReferences` 与 `ThemeDeclaresEachKeyExactlyOnce` 通过。
  - Green：补齐全部令牌；`DarkTheme.xaml` 覆写 6 个新颜色键（间距/圆角/字号/控件尺寸/动效为跨主题常量，只声明一次，深色不覆写）。
  - Refactor：按 UI_DESIGN §2 分节加注释；沿用现有 `<sys:Double>` / `SolidColorBrush` / `<Duration>` / `<CubicEase>` 机制，不引入 `<Color>` 资源或第二套令牌体系。`PageTitleStyle`/`SectionTitleStyle`/`CaptionTextStyle` 的字号由字面量改为令牌引用，消除同义尺寸的第二处定义。
  - **Q3 裁决执行**：移除 `Theme.xaml` 中隐式 `TextBlock` 样式的 `Effect="{DynamicResource UiTextEffect}"`。实测确认该行是全仓**唯一**应用 `Effect` 的位置（grep `Property="Effect"|<TextEffect|Effect=` 仅此一处命中），因此桌面组件的文字描边完全依赖它；删除后桌面组件仍由 `DesktopAppearanceController.cs:66` 把 `UiTextEffect` 写入各自 `window.Resources` 并被同一隐式样式消费，**描边行为不变**；主窗口与 28 个页面则从「解析为空的死引用」变为「无该 Setter」，观感同样不变。契约测试的 `desktopWindowScoped` 白名单同步移除该项。
  - Verify：`eng\verify.cmd` 退出 0；format check 通过；build **0 警告 0 错误**；完整回归 **485/485**（Core 93、Infrastructure 104、Desktop 244、Server 44）= 基线 479 + 本切片新增 6 项；`ThemeTokenContractTests` 6/6；既有 `UiDesignContractTests` 34 项全通过。
  - 约束遵守：**只新增、不改名、不删除**任何既有资源键；本切片未改任何页面 XAML，未新增媒体/图标资源。`IconButtonSize/SidebarWidth/BrandBarHeight/NavigationItemHeight/ControlHeightPrimary/FontClockSize` 目前仅声明未被消费，将在 P60-02/P60-05 接入。

- [x] P60-02 主题控件接入令牌：`Theme.xaml` 内隐式样式与命名样式（含 `PrimaryButtonStyle`、`IconButtonStyle`、`NavigationItemStyle`、`SegmentToggleStyle`）由字面量改用令牌；按 Q1 裁决普通按钮圆角保持 8px（令牌化为 `RadiusLg`，观感零变动）。
  - 实测字面量台账（`Theme.xaml`）：圆角 `8`×4、`6`×4、`4`×8、`3`×3、`2`×3、`5`×1；样式级字号仅 `12`×1（`ToolTip` 样式自身字号，**不是** `CaptionTextStyle` 的重复定义）。
  - Red：新增 2 项契约测试 `ThemeRadiusLiteralsAreTokenized`、`ThemeStyleFontSizesAreTokenized`，首次运行 **2 失败 / 6 通过**，失败原因符合预期（`Theme.xaml` 仍有 `CornerRadius="4|6|8"` 与 `FontSize="12"` 字面量）。
  - Green：圆角 `4→RadiusSm`、`6→RadiusMd`、`8→RadiusLg`，共 15 处改为令牌引用；`ToolTip` 样式字号 `12→FontCaptionSize`。样式级字号字面量清零。
  - **豁免策略**：圆角 `3`（CheckBox 勾选框）/`2`（ProgressBar、Slider 轨道、导航选中条）/`5`（ScrollBar thumb）保留字面量，并在测试中登记到 `StructuralRadiusExemptions`。豁免的唯一理由是「**没有数值相等的令牌可用**」，不是「值很小」——测试同时断言豁免清单与文件内容逐项一致，新增豁免必须显式改测试，避免豁免被滥用成漏改的借口。
  - Refactor：`ToolTip` 样式的字号与圆角同样改为令牌引用；不合并语义不同的样式（`CaptionTextStyle` 面向 TextBlock，`ToolTip` 面向浮层，二者只是数值相同）。
  - **回归与修正（重要）**：首次 `eng\verify.cmd` **失败 15 项**，全部指向同一根因——圆角令牌最初声明为 `sys:Double`，而 XAML 的 double 类型转换器会把 `{DynamicResource RadiusSm}` 解析成**裸 `Double`**，`Border.CornerRadius` 需要 `CornerRadius` 值，于是在 `Border.ArrangeOverride` 抛 `InvalidCastException : Unable to cast object of type 'System.Double' to type 'System.Windows.CornerRadius'`。受影响的是全部 WPF 运行时测试（Assistant/Todo/QuickAdd/RoomBookingPicker）。
    - **测试设计教训**：我最初的契约测试写的是 `light[key] is double`，等于把这个 bug **固化成了期望**，所以 8 项契约测试全绿而 15 项运行时测试全红。已修正为断言 `is CornerRadius`，并新增两项防护：`RadiusTokenTypeMatchesCornerRadiusProperty`（断言具体 CLR 类型）与 `RadiusTokenCanBeAppliedToARealBorder`（把令牌实际赋给真实 `Border` 并强制 `UpdateLayout()`，覆盖「类型对但赋值路径仍失败」的情形）。
    - 修复方式：令牌改用属性语法声明 `<CornerRadius x:Key="RadiusSm">4</CornerRadius>`，复用 WPF 内建的 `CornerRadius` 类型转换器，无需额外 xmlns。
    - 适用范围提示：间距/尺寸/字号令牌消费方是 `double` 属性（`Width`/`Height`/`FontSize`/`Padding` 走 `ThicknessConverter` 或 double），保持 `sys:Double` 正确；**只有圆角需要 `CornerRadius`**。P60-03 若引入 `Thickness` 类令牌需同样注意类型匹配。
...
- [x] P60-05 页面字面量清理：页面标题归位、助理导航换非星光图标、遮罩令牌化、会议窗口标题用语义样式。
  - 实测台账（比方案初版预估的 3 处更多，共 **12 处**页面标题 + 1 处图标 + 1 处遮罩 + 2 处窗口标题）：
    - 内联 `FontSize="20" FontWeight="SemiBold"` 页面标题：`MainWindow.xaml:257`（`{Binding SelectedPageTitle}`，即工作区占位标题）、`Views\CalendarView.xaml:30`（`{Binding DisplayTitle}`）、`Views\ProjectView.xaml:12`、`Views\RecordView.xaml:12`、`Views\TodoView.xaml:186`、`Views\StatisticsView.xaml:14`，以及 `Views\SettingsView.xaml` 的 5 个页签标题（L72 桌面外观 / L316 全局快捷键 / L356 提醒 / L435 AI 模型 / L587 团队连接）。
    - `ViewModels\MainWindowViewModel.cs:26` 助理导航 `PackIconLucideKind.Sparkles`（违反 UI_DESIGN §1.6「AI 降权：不使用星光图标」）。
    - `RegionSelectorWindow.xaml:5` 硬编码 `#33000000`。
    - `MeetingDetailsWindow.xaml:14`、`MeetingExportWindow.xaml:16` 窗口标题内联 `FontSize="18" FontWeight="SemiBold"`。
  - **刻意放行（不是漏改）**：`Views\TodayView.xaml:251` 的天气摘要 `{Binding Weather.CurrentSummary}` 同样写 `FontSize="20"+SemiBold`，但它是**天气大标题**而非页面标题（UI_DESIGN §2.1 允许时钟/天气使用更大字号），契约测试中按绑定名显式放行并注明理由。`RegionSelectorWindow.xaml:18-19` 的 `BorderBrush="White"` 属于选区视觉反馈（截图遮罩上的白色选框），未强行令牌化。
  - Red：在既有 `UiDesignContractTests`（纯源码契约测试）中新增 3 项——`PageTitlesUseTheSharedStyleInsteadOfInlineLiterals`（正则扫描 18 个视图文件，只放行天气摘要）、`AssistantNavigationAvoidsSparkleIconography`、`RegionSelectorScrimUsesTheThemeToken`。首轮 3 项全失败，随后逐项收敛（每修一批就重跑，最终把 12 处标题全部归位）。
  - Green：12 处页面标题改用 `{StaticResource PageTitleStyle}`；助理导航 `Sparkles → Bot`（先用编译探测确认 `Bot/Cpu/Command/Terminal/MessageSquare/Lightbulb` 均存在于 MahApps Lucide 6.2.1，选 `Bot` 因其语义中性、符合「AI 与其他工具同一视觉语言」）；`RegionSelectorWindow` 背景改用 `{DynamicResource OverlayScrimBrush}`；两个会议窗口标题改用 `{StaticResource SectionTitleStyle}`（18px → 16px，顺带消除字号阶梯外的第三档，且这两个窗口高度为 Auto，无截断风险）。
  - Refactor：无。本切片是纯等价替换与规范归位，未改动任何布局结构。
