...
Write-Output ("Window: X=" + $wr.X + " Y=" + $wr.Y + " W=" + $wr.Width + " H=" + $wr.Height + " Right=" + $wr.Right + " Bottom=" + $wr.Bottom)

$sb = New-Object System.Text.StringBuilder
[void]$sb.AppendLine("using System;")
[void]$sb.AppendLine("using System.Runtime.InteropServices;")
[void]$sb.AppendLine("public class MouseSimB {")
$q = [char]34
...
[void]$sb.AppendLine("  [DllImport("+$q+"user32.dll"+$q+", SetLastError=true)] public static extern bool GetCursorPos(out POINT lpPoint);")
[void]$sb.AppendLine("  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int X; public int Y; }")
[void]$sb.AppendLine("  public const uint LEFTDOWN = 0x02, LEFTUP = 0x04, MOVE = 0x0001;")
[void]$sb.AppendLine("  public static POINT GetPos() { POINT p; GetCursorPos(out p); return p; }")
[void]$sb.AppendLine("  public static void ClickAt(int x, int y) { SetCursorPos(x, y); System.Threading.Thread.Sleep(100); POINT cp = GetPos(); System.Console.Error.WriteLine("AfterSet X=" + cp.X + " Y=" + cp.Y); mouse_event(MOVE, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(50); mouse_event(LEFTDOWN, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(50); mouse_event(LEFTUP, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(100); }")
[void]$sb.AppendLine("  public static void DoubleClickAt(int x, int y) { SetCursorPos(x, y); System.Threading.Thread.Sleep(100); mouse_event(MOVE, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(50); mouse_event(LEFTDOWN, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(20); mouse_event(LEFTUP, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(50); mouse_event(LEFTDOWN, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(20); mouse_event(LEFTUP, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(100); }")
[void]$sb.AppendLine("}")
Add-Type -TypeDefinition $sb.ToString() -ErrorAction SilentlyContinue

# Maximize window first? No, but check right edge issue. The CalendarDay cells go X=1133 for Aug30 and window right=1550; click should land inside.
# Target Aug 30 DateText cell center: click the Text "30" at RECT=1147,697,15,19 -> center 1154,706
$tx = 1147 + 7
$ty = 697 + 9
Write-Output (("Click DateText 30 at=" + $tx + "," + $ty))
[MouseSimB]::ClickAt($tx, $ty)
...
Write-Output (("Aug30 selected (by DateText click)=" + $sel))
$ts1 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp1 = "D:/myProgram/cccalendar/screenshots/07_qa_click_aug30_text_" + $ts1 + ".png"
Take-Screenshot -Path $sp1
Write-Output (("SCT_AUG30_TEXT=" + $sp1))
# If still not selected, try SelectionItem pattern Select
if (-not $sel -and $null -ne $target) {
  try {
    ...
    Write-Output (("Aug30 via SelectionItemPattern.Select -> selected=" + $sel2))
    $tsa = Get-Date -Format "yyyyMMdd_HHmmss"
    $spa = "D:/myProgram/cccalendar/screenshots/07_qa_selectitempattern_aug30_" + $tsa + ".png"
    Take-Screenshot -Path $spa
    Write-Output (("SCT_SELECTPATTERN=" + $spa))
  } catch {
    Write-Output (("SelectionItemPattern not supported: " + $_.Exception.Message))
  }
}
# Read the state after attempts
$all2 = $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)
$sel3=$false
foreach ($e in $all2) {
  $n=$e.Current.Name
  if ($n -like "*CalendarDayViewModel*" -and $n -like "*Date = 2026/8/30*" -and $n -like "*IsSelected = True*") { $sel3=$true; break }
}
Write-Output (("FINAL Aug30 selected state=" + $sel3))
# Double click regardless
[MouseSimB]::DoubleClickAt($tx, $ty)
Start-Sleep -Milliseconds 1000
$ts2 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp2 = "D:/myProgram/cccalendar/screenshots/08_qa_dbclick_aug30text_" + $ts2 + ".png"
Take-Screenshot -Path $sp2
Write-Output (("SCT_DBCLICK=" + $sp2))

# Check for quick-add window (ComboBox anywhere)
...
if ($null -eq $Combo) {
  Write-Output "NO_QUICK_WINDOW: no combo after double click"
} else {
  ...
  if ($null -ne $anc) {
    ...
    function DumpB { param($e,$d=0,$md=7) if ($null -eq $e) {return} $pad=""; for($i=0;$i -lt $d;$i++){$pad+="  "} $n=$e.Current.Name; if([string]::IsNullOrEmpty($n)){$n="(empty)"} $t=$e.Current.ControlType.ProgrammaticName -replace "^ControlType..",""; Write-Output ($pad + $t + " " + [char]34 + $n + [char]34); if ($d -ge $md){return} $ch = $e.FindAll([System.Windows.Automation.TreeScope]::Children, [System.Windows.Automation.Condition]::TrueCondition); foreach($c in $ch){ DumpB $c ($d+1) $md } }
    DumpB $anc 0 7
  ...
  if ($null -ne $CancelBtn) {
    ...
    [MouseSimB]::ClickAt([int]($cbR.Left + $cbR.Width/2), [int]($cbR.Top + $cbR.Height/2))
  ...
  $sp3 = "D:/myProgram/cccalendar/screenshots/09_qa_cancel_aug30text_" + $ts3 + ".png"
  Take-Screenshot -Path $sp3
  Write-Output (("SCT_CANCEL=" + $sp3))
}