...
### 2.2 逐项复核（2026-09-16 复核，修正初版的三处过度概括）

初版把「会话壳/气泡/大圆角、渐变光效、星光图标、阅读栏宽度轴」整体划为拒绝。用户指出这些在 DSH 中确实存在、值得考虑。经回查 DSH 源码，结论如下——**其中「渐变光效」一项我的初版判断是错的，已修正**。

| 项 | DSH 中的实际情况（源码证据） | 复核结论 |
|----|------------------------------|----------|
| **会话壳 / 气泡** | 确实存在：`MessageItem.module.css` 气泡 `border-radius:22px`、`InputBar.module.css` 输入容器 `22px`、`ChatView` 三栏壳 | **仍拒绝**，但理由收窄为「产品形态不同」而非「视觉规范冲突」：cccalendar 的助理页是 UI_DESIGN §6 明确定义的「工作记录 + 命令输入」，不是聊天应用；三栏会话壳对九项导航工作台无收益 |
| **大圆角** | 确实存在且是主流：`22`（气泡/输入）、`20`（菜单 `MenuView`、dock 标签）、`24`（弹窗）、`28`（圆形按钮） | **本轮不动控件圆角**（你已确认 8px）。补充说明：DSH 的圆角是**按控件高度算比例**（胶囊 = 高度一半）而非固定值——这是它看起来柔和的原因，不是「大」本身。cccalendar 现有 `8px` 固定值符合 UI_DESIGN §2.2，保持 |
| **渐变** | 存在但**全部是功能性淡出遮罩**，不是装饰：token 层只有 2 个 `--dsw-linear-gradient-think`（思考块的垂直浅渐隐）；全库 37 处 `linear-gradient` 主要是 `mask-image` 边缘淡出（`TurnNavigator`、`ConversationRoot` 滚动提示、标签溢出淡出） | **采纳其功能性用法**：cccalendar 的溢出/滚动提示可改用渐隐遮罩替代硬裁切（候选，见 P60-04 附加项）。**装饰性渐变仍拒绝** |
| **发光 / 光球** | **DSH 中不存在**。实测：`box-shadow:.*glow` **0 命中**、`drop-shadow` **0 命中**、`radial-gradient` 仅 **2 命中**且都是 `boot-page` 的圆环遮罩技巧；`backdrop-filter: blur` 仅 3 处（拖放遮罩、dock 遮罩、引导浮层），用于表达「模态」而非装饰 | **维持拒绝**，但现在有数据支撑：我初版说的「装饰光球」在 DSH 里确实没有，属于 UI_DESIGN §1.2 的既有禁令，与 DSH 无关 |
| **星光图标** | DSH 未使用装饰性星光图标做 AI 品牌（其 AI 身份靠工具调用树、轨迹账本等结构化呈现） | **维持拒绝**：这与 UI_DESIGN §1.6「不使用星光图标」一致，两边立场相同；`MainWindowViewModel.cs:19-30` 的 `Sparkles` 是 cccalendar 自身的违规 |
| **阅读栏宽度轴** | 确实存在：`ConversationRoot.module.css` 的 `--dsh-chat-content-width: clamp(680px, 64%, 920px)` | **维持拒绝**：面向长文阅读，cccalendar 助理页要的是信息密度而非 680–920px 阅读栏；这不是遗漏而是刻意取舍 |

**净修正**：初版的「渐变光效」把功能性遮罩与装饰性渐变混为一谈，属过度概括；现改为「采纳功能性淡出遮罩、拒绝装饰性渐变与发光」。其余三项结论不变，但理由已从「规范冲突」精确到「产品形态/取舍差异」。

### 2.4 其他不与 DSH 对齐的既有约束

以下限制来自 cccalendar 自身规范或技术边界，不是对 DSH 的评价：

| 限制 | 依据 |
|------|------|
| 不引入第三方 UI 组件库 / 不引 Web 前端栈 | ADR-0001；DSH 是 React + CSS Modules，对 WPF 零可移植性 |
| 不使用大面积玻璃/亚克力内容层 | UI_DESIGN §1.2；DSH 的 `backdrop-filter` 仅 3 处且用于模态遮罩，语义不同 |
| 圆角上限 8px（控件） | UI_DESIGN §2.2；你已确认本轮保持 |
| 不照搬 DSH 的 z-index 阶梯 | DSH 自身层级是历史堆积（100/1000/1100 混用，其 `dockkit.module.css` 注释亦承认无令牌层），不值得抄 |
| 间距以 4/8/12/16/24 为准，不采用 DSH 的 2px 增量（1,2,3,5,7…） | UI_DESIGN §2.2 为项目既有基线 |
| 不采用星光 / 渐变头像 /「魔法生成」文案 | UI_DESIGN §1.6「AI 降权」；DSH 本身也未这样做（见 §2.2） |

---

## 3. 现状体检：已确认的问题清单

### 3.1 `UiTextEffect` 资源未定义（已定性 → 决定移除该 Setter）
...
**决定（2026-09-16，用户授权我裁决）：移除 `Theme.xaml:29` 的 `Effect` Setter。**

理由：① 该行当前是无副作用的死引用，删除它**不改变任何现有观感**，而补默认值会让全部 28 个页面的文字突然获得描边——这是一次与 UI 优化目标无关、且用户没要求的视觉突变；② 文字描边实际是 `DesktopAppearanceController` 为桌面组件实现的特性（`ShowTextOutline` 设置项），保留 Setter 在全局主题里属语义错位，桌面窗口仍会通过自身 `window.Resources` 注入并生效，删除后桌面组件的描边**不受影响**；③ 保留这行的唯一后果是让「引用了不存在的键」这一模式继续存在，而 P60-01 要建立的契约测试正是要消灭它。

同时以契约测试固化规则：`Theme.xaml` 中出现的每个 `{DynamicResource X}` 都必须在 `Theme.xaml` 或 `DarkTheme.xaml` 中有声明。`DesktopAppearanceController` 注入的桌面专属键由其自身保证，不在此规则范围（会在测试中显式白名单说明）。
...
| # | 取舍点 | 现状 | 裁决 |
|---|--------|------|------|
| Q1 | 普通按钮圆角 | 模板写死 `CornerRadius="8"`（`Theme.xaml:64`） | **保持 8**（用户确认）——仅令牌化为 `RadiusLg`，观感零变动 |
| Q2 | 会议室看板可见时段 | 08:00–24:00（`FirstVisibleCell=16`，576px） | **保持 08:00–24:00**（用户确认）——本轮只换皮，不动信息架构 |
...
## 8. 裁决结果（2026-09-16 已定，不再阻塞）

| # | 问题 | 裁决 | 执行方式 |
|---|------|------|----------|
| Q1 | 普通按钮圆角 8px vs 4px | **保持 8px**（用户确认） | P60-02 仅把它令牌化为 `RadiusLg`，观感零变动 |
| Q2 | 会议室看板可见时段 | **保持 08:00–24:00**（用户确认） | `FirstVisibleCell=16`、`VisibleCells=32` 不变；P60-03 只换皮不动时段 |
| Q3 | `UiTextEffect` 未定义 | **移除 `Theme.xaml:29` 的 Setter**（用户授权我裁决，理由见 §3.1） | P60-01 随契约测试一并处理；桌面组件描边不受影响 |
| Q4 | 切片顺序 P60-01→P60-06 | **通过**（用户要求持续推进、不必逐项确认） | 每片完成即更新 `WORKLIST.md`；用户看效果后再提意见 |
