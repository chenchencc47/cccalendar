...
[void]$sb.AppendLine("public class MouseSim3 {")
$q = [char]34
...
[void]$sb.AppendLine("  public static void Click(int x, int y) { SetCursorPos(x, y); System.Threading.Thread.Sleep(30); mouse_event(LEFTDOWN, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(20); mouse_event(LEFTUP, 0, 0, 0, UIntPtr.Zero); System.Threading.Thread.Sleep(30); }")
...
# Find Sep5 CalendarDayViewModel DataItem from root
$all = $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)
$target = $null
foreach ($e in $all) {
  $n = $e.Current.Name
  if ($n -like "*CalendarDayViewModel*" -and $n -like "*Date = 2026/9/5*") { $target = $e; break }
}
if ($null -eq $target) { Write-Output "FAIL: Sep5 not found"; exit 1 }
$r = $target.Current.BoundingRectangle
Write-Output (("Sep5 RECT=X=" + $r.X + " Y=" + $r.Y + " W=" + $r.Width + " H=" + $r.Height + " -> RX=" + $r.Right + " BY=" + $r.Bottom))
$tx = [int]($r.Left + $r.Width/2)
$ty = [int]($r.Top + [int]($r.Height * 0.3))
Write-Output (("Click point=" + $tx + "," + $ty))
[MouseSim3]::Click($tx, $ty)
Start-Sleep -Milliseconds 500
# Verify selected
$n2 = $target.Current.Name
$sel = if ($n2 -like "*IsSelected = True*") { $true } else { $false }
Write-Output (("IsSelected after click=" + $sel + " NameSnippet=" + $n2.Substring(0, [Math]::Min(180, $n2.Length))))
$ts1 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp1 = "D:/myProgram/cccalendar/screenshots/07_qa_click_sep5_ok_" + $ts1 + ".png"
Take-Screenshot -Path $sp1
Write-Output (("SCREENSHOT_CLICK_OK=" + $sp1))
# DoubleClick at same point
[MouseSim3]::DoubleClick($tx, $ty)
Start-Sleep -Milliseconds 1000
$ts2 = Get-Date -Format "yyyyMMdd_HHmmss"
$sp2 = "D:/myProgram/cccalendar/screenshots/08_qa_doubleclick_quickadd_sep5v2_" + $ts2 + ".png"
Take-Screenshot -Path $sp2
Write-Output (("SCREENSHOT_DBCLICK_V2=" + $sp2))
...
if ($null -eq $Combo) {
  Write-Output "NO_QUICK_WINDOW: v2 no window"
} else {
  Write-Output "QUICK_WINDOW_OPENED: v2 yes"
  # Inspect combo text
  ...
  if ($null -ne $anc) {
    ...
    function Dump3 { param($e,$d=0,$md=7) if ($null -eq $e) {return} $pad = ""; for($i=0;$i -lt $d;$i++){$pad += "  "} $n=$e.Current.Name; if([string]::IsNullOrEmpty($n)){$n="(empty)"} $t=$e.Current.ControlType.ProgrammaticName -replace "^ControlType..",""; Write-Output ($pad + $t + " " + [char]34 + $n + [char]34); if ($d -ge $md){return} $ch = $e.FindAll([System.Windows.Automation.TreeScope]::Children, [System.Windows.Automation.Condition]::TrueCondition); foreach($c in $ch){ Dump3 $c ($d+1) $md } }
    Dump3 $anc 0 7
  ...
  if ($null -ne $CancelBtn) {
    ...
    [MouseSim3]::Click([int]($cbR.Left + $cbR.Width/2), [int]($cbR.Top + $cbR.Height/2))
  ...
  $sp3 = "D:/myProgram/cccalendar/screenshots/09_qa_after_cancel_v2_" + $ts3 + ".png"
  Take-Screenshot -Path $sp3
  Write-Output (("SCREENSHOT_CANCELLED_V2=" + $sp3))
}